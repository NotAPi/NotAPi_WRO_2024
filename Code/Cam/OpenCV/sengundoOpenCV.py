...
while True:
    ...
    # Función para calcular y dibujar el centroide usando momentos
    def draw_center_of_mask(mask, image, color):
        # Calcular los momentos de la máscara
        M = cv2.moments(mask)
        
        # Solo calcular el centroide si el área es significativa
        if M["m00"] > 100:
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            ...
            cv2.line(image, (0, cY), (anchura // reduccion, cY), color, 1)
            cv2.line(image, (cX, 0), (cX, altura // reduccion), color, 1)

    # Calcular y dibujar el centroide para las máscaras roja y verde en la imagen redimensionada
    draw_center_of_mask(red_mask_resized, original_resized, (0, 0, 255))  # Rojo
    draw_center_of_mask(green_mask_resized, original_resized, (0, 255, 0))  # Verde
